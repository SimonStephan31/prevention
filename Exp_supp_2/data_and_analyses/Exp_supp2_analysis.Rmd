---
title: "Preemption in Prevention Supp. 2"
output:  
  html_document:
...
```{r}
# read data 
tdata <- read.table("Exp_supp2_data.txt", header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
...
tdata$test_case <- factor(tdata$test_case, levels = c("one_too_late", "both_on_time"), labels = c("Immediate effect", "Delayed effect"))
...
CI_low <- c(CI_both_min_1, CI_first_min_1, CI_both_min_2, CI_first_min_2)

CI_up <- c(CI_both_max_1, CI_first_max_1, CI_both_max_2, CI_first_max_2)
...
g
#ggsave("Exp3c_selections.pdf",width=7.5,height=4)
```
